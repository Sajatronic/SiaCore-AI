...
-- Distinct lifecycle statuses, for populating the filter dropdown.
...
as $$
  ...
  -- Real alternative-sourcing risk, from the alternatives_risk table
  -- (PARS_category is a text bucket like 'High'/'Medium'/'Low' describing
  -- how well-covered this part is by alternates — replaces the earlier
  -- placeholder that derived a fake "risk level" purely from a row count).
  latest_alt_risk as (
...
-- get_part_stock_suppliers — per-distributor stock table for Part Details.
-- Drop every existing overload of get_part_stock_suppliers, whatever its old
-- signature was, rather than guessing one specific old parameter list.
-- drop function if exists get_part_stock_suppliers(text, date, date);
do $$
declare
  r record;
begin
  for r in
    select oid::regprocedure as sig
    from pg_proc
    where proname = 'get_part_stock_suppliers'
      and pronamespace = 'public'::regnamespace
  loop
    execute format('drop function %s;', r.sig);
  end loop;
...
-- ---------------------------------------------------------------------
-- 3. INVENTORY RECORDS SCREEN
-- ---------------------------------------------------------------------
-- Drop every existing overload of get_inventory_records, whatever its old
-- signature was, rather than guessing one specific old parameter list.
...
begin
  for r in
    select oid::regprocedure as sig
    from pg_proc
    where proname = 'get_inventory_records'
      and pronamespace = 'public'::regnamespace
  loop
    execute format('drop function %s;', r.sig);
  end loop;
...
stable
as $$
  select forecast_date, forecast_demand, safety_stock, reorder_point, min_stock, max_stock
...
declare
  ...
  tables text[] := array[
    ...
    'stock', 'inventory_forecast', 'inventory_risk', 'agent_logs',
