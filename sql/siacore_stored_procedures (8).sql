...
begin
  ...
  begin
    select count(*) into v_risk_events from risk_events where status::text is distinct from 'resolved';
...
as $$
  ...
  from risk_events
  where status::text is distinct from 'resolved'
...
as $$
  select
    ...
    count(*)::int                                as count
  from risk_events re
  where re.status::text is distinct from 'resolved'
...
stable
as $$
  select distinct s."Manufacturer_Lifecycle_Status"
  from stock s
  where s."Manufacturer_Lifecycle_Status" is not null
  order by 1;
$$;
...
stable
as $$
  -- p_start/p_end are accepted for signature compatibility with the
  -- frontend's calendar filter, but deliberately NOT applied to any of
  -- the CTEs below. This page shows the CURRENT risk/stock picture for
  -- one part, not a historical trend — every value here should always
  -- be "whatever the most recent row says," full stop, regardless of
  -- what date range happens to be selected. Applying the date filter
  -- here was the reason fields like Lifecycle Status could silently
  -- come back empty: the one row that existed for this mpn just
  -- happened to fall outside whatever range was picked.
  with latest_stock_risk as (
  ...
  -- True current snapshot: find the most recent inventory date for this
  -- mpn, then sum qty_on_hand across whatever warehouses reported on
  -- that exact date — not a running total across all history (which
  -- would double-count every prior day this part was ever inventoried).
  latest_inventory_date as (
    select max(i.date) as max_date from inventory i where i.mpn = p_mpn
  ),
  latest_inventory as (
    ...
    join latest_inventory_date lid on i.date = lid.max_date
    where i.mpn = p_mpn
    group by i.mpn
  ),
  latest_inventory_risk as (
...
as $$
  ...
  order by
    -- Authorized distributors first (true sorts before false with asc
    -- on a boolean-style expression), then most recently scraped within
    -- each of those two groups.
    (lower(coalesce(st."Authorized_Dist"::text, '')) in ('yes', 'y', 'true', 'authorized', '1')) desc,
    st.scraped_at desc;
$$;
...
-- Return type changed back (removed total_extended_value) — still needs
-- the safe drop-and-recreate since the column list changed again.
do $$
declare r record;
begin
  for r in select oid::regprocedure as sig from pg_proc
